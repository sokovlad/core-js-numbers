...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const result = Math.sqrt(Math.abs(x2 - x1) ** 2 + Math.abs(y2 - y1) ** 2);
  return result;
}
...
function toNumber(value, def) {
  ...
  let result;
  if (Number.isInteger(valueToNum)) {
    result = valueToNum;
  } else {
    result = def;
  }
...
function getSumOfDigits(num) {
  const numToStr = String(num).split('');
  let result = 0;
...
function isPowerOfTwo(num) {
  ...
  const result =
    Math.ceil(Math.log(num) / Math.log(2)) ===
...
function isNumber(number) {
  const isNum = Number.isFinite(number);
  return isNum;
}
